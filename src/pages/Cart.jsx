  ...
  if (cartItems.length === 0) {
    return (
      <div className="p-72 text-center text-4xl">
        🛒 Your cart is empty.{" "}
        <Link to="/collection/all" className="font-bold underline block mt-10">
          CONTINUE SHOPPING
        </Link>
      </div>
    );
    ...
    <div className="container py-12">
      ...
      <div className="grid md:grid-cols-3 gap-4 sm:gap-8 md:gap-16 mt-4">
        <div className="md:col-span-2 mt-5">
          {cartItems.map((item) => (
            <CartItem
              key={`item.id-${JSON.stringify(item.selectedOption)}`}
              item={item}
              onUpdate={handleUpdateQuantity}
              onRemove={removeFromCart}
              error={
                errorQuantities[
                  `${item.id}-${JSON.stringify(item.selectedOption)}`
                ]
              }
            />
          ))}
        ...
        <div className="bg-gray-100 rounded shadow-lg p-5  h-fit">
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-gray-500 text-xl font-light">Total</span>
          ...
          <div className="flex items-center justify-between mt-4 text-2xl font-bold">
            ...
            <span className="">{currencyFormatter.format(totalPrice)}</span>
          </div>
          <button
          ...
          <p className="text-xs text-gray-400 mt-4">
            Newlife processes all orders in USD. Shipping & taxes calculated at
            checkout.
          </p>
        </div>
      </div>
